...
class DefFile(object):
        ...
        for d in self.definitions:
            if d.type == dfn.type and d.name == dfn.name:
                if quiet:
                    ...
                    raise TypeError(
                        "%s:%s definition already exists" % (dfn.type, dfn.name))
    ...
    def tojson(self):
        ...
        d['Definitions'] = dict(
            [(dfn.type + ':' + dfn.name, dfn.tojson()) for dfn in self.definitions])

        d['Sections'] = dict([(section_name, section.tojson())
    ...
    def get(self, key, default=None):
        if isinstance(key, tuple):
            ...
            null_sec_val = self.get_section(None).__getitem__(key) if self.has_section(
                None) and hasattr(self.get_section(None), '__getitem__') else None
            if (self.manifest and key in self.manifest
                    and null_sec_val is not None):
                raise RuntimeError(
                    'Manifest and null section both have key %s!' % str(key))
...
class Manifest(object):
    ...
    def __repr__(self):
        def_type_str = ',%s' % repr(
            self.def_type) if self.def_type is not None else ''
...
class INISection(Section):
    ...
    def __init__(self, name, fields=None, discard_none=False):
        ...
        self.fields = strstrdict((key, value) for key, value in fields.iteritems(
        ) if not (value is None and discard_none)) if fields else strstrdict()
...
class DefFileParser(object):
    ...
    # r'[^\s=](?:\s*\S)*'
    BASIC_KEY_PATT = r'{char}+(?:\s+{char}+)*'.format(char=BASIC_KEY_CHAR_PATT)
    BASIC_VALUE_PATT = r'(?:\S(?:\s*\S)*)?'
    BASIC_KEY_VALUE_PATT = KEY_VALUE_PATT_TEMPLATE.format(
        key_patt=BASIC_KEY_PATT, value_patt=BASIC_VALUE_PATT)
    ...
    def get_section_parser(self, name, format, def_file_format, go_easy=False):
        ...
        if not matches:
            if go_easy:
                ...
                raise ParsingError(
                    'No section parser for section %s and format %s' % (name, format))
            else:
    ...
    def get_definition_parser(self, dfn_type, name, format, def_file_format, go_easy=False):
        ...
        for key, parser in self.definition_parsers.iteritems():
            ...
            matches[score] = parser
        if not matches:
            parser = self.get_section_parser(
                None, format, def_file_format, go_easy=True)
            if parser:
        ...
        if not matches:
            if go_easy:
                ...
                raise ParsingError(
                    'No definition parser for type %s and format %s' % (dfn_type, format))
            else:
                raise ParsingError(
                    'No definition parser for type %s' % dfn_type)
        ...
        while line is not None:
            ...
            if not section_info:
                if self.null_section and self.null_section_parser:
                    parser = self.null_section_parser(
                        None, None, self._get_reader())
                    sec = parser.parse()
                    section_info = {'section': None, 'data': sec}
                else:
                    raise ParsingError(
                        'Parsing error: invalid section line\n%s' % line)
    ...
    def parse(self, fp, **kwargs):
        ...
        for section in self.sections():
            if section['section'] is None:
                ...
                if def_file.definitions or def_file.sections:
                    raise ParsingError(
                        'Parsing error: manifest section must be first!')
                ...
                parser = DefFileParser.ManifestParser(
                    section['section'], None, self._get_reader())
                def_file.manifest = parser.parse()
            elif section['dfn_type']:
                parser = self.get_definition_parser(
                    section['dfn_type'], section['section'], section['format'], def_file.format)
                definition = parser.new_section()
                parser.parse(definition)
                def_file.definitions.append(definition)
            else:
                parser = self.get_section_parser(
                    section['section'], section['format'], def_file.format)
        ...
        if self.require_def_type and not re.match(self.require_def_type, def_file.type):
            raise ParsingError("Require def file type %s, got %s" %
                               (self.require_def_type, def_file.type))
        self.fp = None
        self.current_line = 0
        return def_file
    ...
    class DefinitionParser(SectionParser):
        ...
        def from_section_parser(section_parser):
            ...
            class DfnParser(section_parser, DefFileParser.DefinitionParser):
                def __init__(self, dfn_type, name, format, reader):
                    DefFileParser.DefinitionParser.__init__(
                        self, dfn_type, name, format, reader)
    ...
    class ManifestParser(Subparser):
        def parse(self):
            ...
            for line in self.reader():
                ...
                if not match:
                    raise ParsingError(
                        'Parsing error: invalid manifest line\n{line}'.format(line=line))
...
class INISectionParser(DefFileParser.SectionParser):
    ...
    def parse(self, section):
        line_patt = DefFileParser.KEY_VALUE_PATT_TEMPLATE.format(
            key_patt=self.KEY_PATT, value_patt=self.VALUE_PATT)
        ...
        for line in self.reader():
            ...
            if not match:
                raise ParsingError(
                    'Parsing error: Invalid INI section line\n{line}'.format(line=line))
    ...
    def parse(self, section):
        ...
        for line in self.reader():
            ...
            if extended_key is not None:
                if match:
                    extended_data = re.sub(
                        r'\\=', r'=', extended_joiner.join(extended_data))
                    ...
                    if not extended_match:
                        raise ParsingError('Parsing error: Invalid Extended INI section line for key {key}:\n{line}'.format(
                            key=extended_key, line=line))
                    extended_data.append(line)
                    continue
            if not match:
                raise ParsingError(
                    'Parsing error: Invalid Extended INI section line\n{line}'.format(line=line))
            key = re.sub(r'\\=', r'=', match.group('DefFileParser_key'))
            value = match.group('DefFileParser_value')
            if value == '|':
                extended_key = key
...
class RawSectionParser(DefFileParser.SectionParser):
    ...
    def parse(self, section):
        ...
        if not re.match(self.CONTENT_PATT, content, flags=re.MULTILINE):
            raise ParsingError(
                'Parsing error: invalid raw section\n{content}'.format(content=content))
...
INIDefinitionParser = get_definition_parser_from_section_parser(
    INISectionParser)
ExtendedINIDefinitionParser = get_definition_parser_from_section_parser(
    ExtendedINISectionParser)
...
class ROSStyleDefinitionParser(DefFileParser.DefinitionParser):
    ...
    def parse(self, rosdef):
        ...
        for line in self.reader():
            ...
            if not match:
                raise ParsingError(
                    'Parsing error: invalid ROS-style definition line\n{line}'.format(line=line))
            segment.append(ROSStyleDefinition.Field(name=match.group(
                'ROSStyleDefinitionParser_name'), type=match.group('ROSStyleDefinitionParser_type')))
        if seg_line_change:
        ...
        if num_segments and len(segs) != num_segments:
            raise ParsingError(
                'Parsing error: This ROS-style definition is required to have %d segments: %s' % (num_segments, self.SEGMENT_NAMES))
