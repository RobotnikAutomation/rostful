...
class Service:
    ...
    def call(self, rosreq):
        ...
        try:
            ...
            rospy.logerr(
                'RostfulServer::Service::call: Error calling service %s: %s', self.name, e)
            return False
    ...
    def __init__(self, topic_name, topic_type, allow_pub=True, allow_sub=True, queue_size=10):
        ...
        if self.allow_sub:
            self.sub = rospy.Subscriber(
                self.name, self.rostype, self.topic_callback)
...
class Action:
    ...
    def __init__(self, action_name, action_type, queue_size=1):
        ...
        self.rostype_action_goal = getattr(
            msg_module, action_type_name + 'ActionGoal')
        self.rostype_action_result = getattr(
            msg_module, action_type_name + 'ActionResult')
        self.rostype_action_feedback = getattr(
            msg_module, action_type_name + 'ActionFeedback')

        self.rostype_goal = getattr(msg_module, action_type_name + 'Goal')
        self.rostype_result = getattr(msg_module, action_type_name + 'Result')
        self.rostype_feedback = getattr(
            msg_module, action_type_name + 'Feedback')

        self.status_msg = deque([], queue_size)
        self.status_sub = rospy.Subscriber(
            self.name + '/' + self.STATUS_SUFFIX, actionlib_msgs.msg.GoalStatusArray, self.status_callback)

        self.result_msg = deque([], queue_size)
        self.result_sub = rospy.Subscriber(
            self.name + '/' + self.RESULT_SUFFIX, self.rostype_action_result, self.result_callback)

        self.feedback_msg = deque([], queue_size)
        self.feedback_sub = rospy.Subscriber(
            self.name + '/' + self.FEEDBACK_SUFFIX, self.rostype_action_feedback, self.feedback_callback)

        self.goal_pub = rospy.Publisher(
            self.name + '/' + self.GOAL_SUFFIX, self.rostype_action_goal, queue_size=10)
        self.cancel_pub = rospy.Publisher(
            self.name + '/' + self.CANCEL_SUFFIX, actionlib_msgs.msg.GoalID, queue_size=10)
...
def get_suffix(path):
    suffixes = '|'.join([re.escape(s)
                        for s in [CONFIG_PATH, SRV_PATH, MSG_PATH, ACTION_PATH]])
...
class RostfulServer:
    ...
    def add_service(self, service_name, ws_name=None, service_type=None):
        ...
        if service_type is None:
            ...
            if not service_type:
                rospy.logdebug(
                    'RostfulServer:add_service: Unknown service %s', service_name)
                return False
    ...
    def add_services(self, service_names):
        ...
        for service_name in service_names:
            ...
            if service_name_key not in self.services:
                ...
                if ret:
                    rospy.loginfo(
                        'RostfulServer::add_services: added %s', service_name)
    ...
    def add_topic(self, topic_name, ws_name=None, topic_type=None, allow_pub=True, allow_sub=True):
        ...
        rospy.loginfo('RostfulServer: add_topic: topic_name=%s, resolved_topic_name = %s',
                      topic_name, resolved_topic_name)
        if topic_type is None:
            topic_type, _, _ = rostopic.get_topic_type(resolved_topic_name)
            if not topic_type:
                rospy.logdebug(
                    'RostfulServer: add_topic: Unknown topic %s', topic_name)
                return False
        ...
        try:
            self.topics[ws_name] = Topic(
                topic_name, topic_type, allow_pub=allow_pub, allow_sub=allow_sub)
        except Exception as e:
            rospy.logdebug("RostfulServer::add_topic: Error creating Topic for %s:%s -> %s" % (
                topic_name, topic_type, e))
            return False
    ...
    def add_topics(self, topic_names, allow_pub=True, allow_sub=True):
        ...
        for topic_name in topic_names:
            ...
            if topic_name_key not in self.topics:
                ret = self.add_topic(
                    topic_name, allow_pub=allow_pub, allow_sub=allow_sub)
                if ret:
                    rospy.loginfo(
                        'RostfulServer::add_topics: added topic %s', topic_name)
    ...
    def add_action(self, action_name, ws_name=None, action_type=None):
        if action_type is None:
            ...
            if not topic_type:
                rospy.logdebug(
                    'RostfulServer::add_action: Unknown action %s', action_name)
                return False
    ...
    def add_actions(self, action_names):
        ...
        for action_name in action_names:
            ...
            if action_name_key not in self.actions:
                ...
                if ret:
                    rospy.loginfo(
                        'RostfulServer::add_actions: added action %s', action_name)
    ...
    def _handle_get(self, environ, start_response):
        ...
        if path == CONFIG_PATH:
            dfile = definitions.manifest(
                self.services, self.topics, self.actions, full=full)
            if jsn:
        ...
        if not suffix:
            ...
            if msg is None:
                rospy.logerr(
                    'RostfulServer::_handle_get: topic %s not available', topic_name)
        ...
        if suffix == CONFIG_PATH:
            if path in self.services:
                ...
                dfile = definitions.describe_topic(
                    topic_name, topic, full=full)
                ...
                action = self.actions[action_name]
                dfile = definitions.describe_action(
                    action_name, action, full=full)
    ...
    def _handle_post(self, environ, start_response):
        ...
        try:
            ...
            if name in self.services:
                ...
                print('service = ', service, 'type = ', input_msg_type)
            ...
            if use_ros:
                ...
                input_data.pop('_format', None)
                msgconv.populate_instance(input_data, input_msg)
                print('input_data', input_data, 'input_msg', input_msg)
            ...
            if mode == 'service':
                rospy.logwarn(
                    'RostfulServer::_handle_post: calling service %s with msg : %s', service.name, input_msg)
                ret_msg = service.call(input_msg)
            elif mode == 'topic':
                rospy.logwarn(
                    'RostfulServer::_handle_post: publishing -> %s to topic %s', input_msg, topic.name)
                topic.publish(input_msg)
                return response_200(start_response, [], content_type=content_type)
            elif mode == 'action':
                rospy.logwarn(
                    'RostfulServer::_handle_post: publishing %s to action %s', input_msg, action.name)
...
def servermain():
    ...
    parser.add_argument('--services', '--srv', nargs='+',
                        help='Services to advertise')
    parser.add_argument('--topics', nargs='+',
                        help='Topics to both publish and subscribe')
    parser.add_argument('--publishes', '--pub', nargs='+',
                        help='Topics to publish via web services')
    parser.add_argument('--subscribes', '--sub', nargs='+',
                        help='Topics to allowing publishing to via web services')
    parser.add_argument('--actions', nargs='+', help='Actions to advertise')

    parser.add_argument('--host', default='')
    parser.add_argument('-p', '--port', type=int, default=8080)
    parser.add_argument('--rest-prefix', default='/',
                        help='The prefix path of the http request, usually starting and ending with a "/".')
    ...
    parser.add_argument('--jwt-alg', default='HS256',
                        help='This arguments sets the algorithm to encode/decode the data')
    parser.add_argument('--ipv6', action='store_true', default=False,
                        help="This argument starts the server with an IPv6 instead of IPv4")
    ...
    try:
        ...
        if args.ipv6:
            httpd = make_server(args.host, args.port,
                                server.wsgifunc(), server_class=WSGIServerV6)
        else:
            httpd = make_server(args.host, args.port, server.wsgifunc())

        rospy.loginfo('rostful_server: Started server on  %s:%d',
                      args.host, args.port)
